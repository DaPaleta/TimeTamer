...
export const TaskDetailsDialog: React.FC<TaskDetailsDialogProps> = ({
  ...
  const handleEditDialogClose = async (updatedTask?: Task) => {
    ...
    if (updatedTask && task) {
      ...
      if (
        updatedTask.estimated_duration_minutes !== task.estimated_duration_minutes &&
        task.scheduled_slots &&
        task.scheduled_slots.length > 0
      ) {
        const slot = task.scheduled_slots[0]
        const startTime = new Date(slot.start_time)
        const newEndTime = new Date(
          startTime.getTime() + updatedTask.estimated_duration_minutes * 60000
        )
      ...
      >
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h6">{loading ? 'Loading...' : task?.title}</Typography>
        ...
        <DialogActions>
          <Button startIcon={<EditIcon />} onClick={handleEditTask} disabled={loading}>
            Modify Task
          </Button>
          <Button
