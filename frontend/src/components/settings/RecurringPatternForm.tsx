...
import type { UserDaySetting, UserDaySettingCreate, UserDaySettingUpdate, RecurrencePattern } from '../../api/calendar';
...
export const RecurringPatternForm: React.FC<RecurringPatternFormProps> = ({
  ...
  const [settingType, setSettingType] = useState<'work_environment' | 'focus_slots' | 'availability_slots'>(
  ...
  const [endDate, setEndDate] = useState(
    initialData?.recurrence_pattern.end_date || ''
  );
  const [interval, setInterval] = useState(
    initialData?.recurrence_pattern.interval || 1
  );
  ...
  const [focusSlots, setFocusSlots] = useState<Array<{
    start_time: string;
    end_time: string;
    focus_level: 'high' | 'medium' | 'low';
  }>>(
    (initialData?.value as any)?.focus_slots || [{ start_time: '09:00', end_time: '11:00', focus_level: 'high' }]
  );
  ...
  const [availabilitySlots, setAvailabilitySlots] = useState<Array<{
    ...
    (initialData?.value as any)?.availability_slots || [{ start_time: '09:00', end_time: '17:00', status: 'available' }]
  );
  ...
  const addAvailabilitySlot = () => {
    setAvailabilitySlots([...availabilitySlots, { start_time: '09:00', end_time: '17:00', status: 'available' }]);
  };
