...
export const useDayContextEventSource = ({
  ...
  const eventClassNames = useCallback((arg: { event: { extendedProps: DayContextEvent['extendedProps'] } }) => {
...
export const useDayContextEvents = (dayContexts: CalendarDayContext[]) => {
  ...
  const [selectedEventType, setSelectedEventType] = React.useState<'work_environment' | 'focus_slot' | null>(null);
  const [selectedEventData, setSelectedEventData] = React.useState<Record<string, unknown> | null>(null);
