...
const CategoryManagerPage: React.FC = () => {
  ...
  const handleEditSave = async () => {
    ...
    setError(null)
    try {
      await api.put(`/tasks/categories/${editDialog.category_id}`, {
        name: editName,
        color_hex: editColor,
      })
      setEditDialog(null)
  ...
  return (
    <Box maxWidth="sm" mx="auto" mt={4}>
        ...
        <List>
          {categories.map((cat) => (
            <ListItem key={cat.category_id} sx={{ borderBottom: '1px solid #eee' }}>
                ...
                primary={
                  <>
                    <Chip
                      size="small"
                      label={cat.name}
                      sx={{ bgcolor: cat.color_hex, color: '#fff', mr: 1 }}
                    />
                    {cat.name}
                  </>
                }
              ...
              />
              <ListItemSecondaryAction>
                <IconButton edge="end" onClick={() => openEdit(cat)}>
                  <EditIcon />
                </IconButton>
                <IconButton
                  edge="end"
                  ...
                  disabled={deleteId === cat.category_id && deleteLoading}
                >
                  {deleteId === cat.category_id && deleteLoading ? (
      ...
      <Dialog open={!!editDialog} onClose={() => setEditDialog(null)}>
        ...
        <DialogActions>
          <Button onClick={() => setEditDialog(null)} disabled={editSaving}>
            Cancel
          </Button>
          <Button
