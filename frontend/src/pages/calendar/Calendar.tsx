...
export const MyCalendar = () => {
  ...
  const handleDialogDelete = async (
    ...
    try {
      ...
      const updatedFocusSlots = currentDay.focus_slots.filter(
  ...
  const handleEventClick = (clickInfo: {
    ...
    if (eventType === "work_environment") {
      ...
      console.log(
        `Work environment clicked: ${eventDate} - ${environment} (${eventSource})`
      );
    ...
    const taskId =
      (extendedProps.taskId as string) || (extendedProps.task_id as string);
    if (taskId) {
  ...
  const handleEventReceive = async (info: EventReceiveInfo) => {
    ...
    const taskId =
      ...
      (info.event.extendedProps.task_id as string);
    ...
    try {
      await updateTask(taskId, {
        scheduled_slots: [
          { start_time: startTime, end_time: endTime, calendar_day_id: null },
        ],
      });
  ...
  const handleEventDrop = async (info: EventDropArg) => {
    ...
    const taskId =
      (event.extendedProps.taskId as string) ||
      (event.extendedProps.task_id as string);
  ...
  const renderEventContent = (arg: EventContentArg) => {
    ...
    const eventDate = extendedProps.date as string;
    const eventSource = extendedProps.source as string;
    const isDayContextEvent =
      eventType === "work_environment" || eventType === "focus_slot";
    ...
    const handleRightClick = (e: React.MouseEvent) => {
      ...
      if (eventType === "focus_slot") {
        ...
        const dayContext = calendarContext.find(
